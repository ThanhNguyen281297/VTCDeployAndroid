...
local autoCount = 0;
this.isMaxRolling = false;
this.hasUsedFreeReset = false;
local autoMaxRollNextTime = 0;
...
function UISlotMachine.Update()
  if (this.lastDebugLogTime2 or 0) < CGTimer.time then
    logError("[MaxRollDebugUnconditional] Update is RUNNING!")
    this.lastDebugLogTime2 = CGTimer.time + 1.0
  end
  ...
  if UISlotMachine.isMaxRolling then
    if CGTimer.time > (this.lastDebugLogTime or 0) then
      logError(string.format("[MaxRollDebug] state=%s coin=%s", tostring(state), tostring(this.coin)))
      this.lastDebugLogTime = CGTimer.time + 1.0
    end
  end

  if UISlotMachine.isMaxRolling and state == ESlotMachineState.None and this.coin > 0 then
    this.ExecuteMaxRollLogic()
  end
  ...
  if state == ESlotMachineState.Run then
    ...
    elseif currentSlot > targetSlot - breakSlots then
      AudioManager.PlayChannelOnce("WA0094", 8 + math.fmod(currentSlot, 3), 0.3);
    ...
    --轉動
      ...
      AudioManager.PlayChannelOnce("WA0094", 8 + math.fmod(currentSlot, 3), 0.25);
      slots[currentSlot].image_Frame.enabled = true;
...
function UISlotMachine.OnClick_Probability()
  if auto and state ~= ESlotMachineState.None then
  ShowCenterMessage(string.Get(71218));  --轉動中請稍候
  return;
  end
...
-- =========================================================================
-- VTC MOD: MAXROLL AI BOT (Logic by Antigravity)
-- =========================================================================
function UISlotMachine.GetBoardEV()
  local totalEV = 0
  local xuVangSlots = 0
  
  for i = 1, this.maxSlotItem do
    local award = this.awards[i]
    if award ~= nil and award.itemId == 31171 then
      xuVangSlots = xuVangSlots + 1
      
      -- Tính chính xác số lượng Xu Vàng y hệt cách UI hiển thị
      local count = award.count
      local itemData = itemDatas[award.itemId]
      if itemData ~= nil and itemData:IsDirectUse() then
        count = itemData.attribute[1].value * award.count
      end
      
      -- Xử lý xác suất (rate)
      local rate = award.pr or 0
      
      -- Auto-detect percentage formats
      if rate > 100 then
        rate = rate / 10000
      elseif rate > 1 then
        rate = rate / 100
      ...
      totalEV = totalEV + (count * rate)
    end
  end
  
  -- Optimization/Safety: Nếu không có ô Xu Vàng nào, EV chắc chắn bằng 0
  if xuVangSlots == 0 then 
    return 0 
  end
...
function UISlotMachine.ExecuteMaxRollLogic()
  if not UISlotMachine.isMaxRolling then 
    logError("[MaxRoll] FAIL: isMaxRolling is false")
    return 
  end
  if state ~= ESlotMachineState.None then 
    logError("[MaxRoll] FAIL: state is not None. Current state=" .. tostring(state))
    return 
  end
  if this.coin == 0 or table.Count(this.awards) == 0 then 
    logError("[MaxRoll] FAIL: coin=" .. tostring(this.coin) .. " awardsCount=" .. tostring(table.Count(this.awards)))
    return 
  end
  if CGTimer.time < autoMaxRollNextTime then 
    -- logError("[MaxRoll] FAIL: waiting for timer. time=" .. CGTimer.time .. " next=" .. autoMaxRollNextTime)
    return 
  end
  
  local ev = this.GetBoardEV()
  if ev < 0.5 then
    if not this.hasUsedFreeReset then
      logError(string.format("[MaxRoll] Ban Rac (EV=%.2f). Thuc hien Reset Mien phi.", ev))
      this.hasUsedFreeReset = true
      UISell.Launch(41)
      autoMaxRollNextTime = CGTimer.time + 1.0
    else
      logError(string.format("[MaxRoll] Ban Rac (EV=%.2f). Het Quyen Reset -> EP Quay X1.", ev))
      this.hasUsedFreeReset = false
      requireSpace = 2;
      requireCoin = 1;
      protocolSubKind = 2;
      ...
      if this.CheckTurn() then 
        this.isMaxRolling = false 
        if CGTimer then CGTimer.RemoveListener(this.ExecuteMaxRollLogic) end
        if this.OnClick_Stop then pcall(this.OnClick_Stop) end
        if UIDebug and UIDebug.UpdateUI then UIDebug.UpdateUI() end
        return 
      end
      this.SendStartTurn()
    end
    ...
    if ev >= 1.0 then
      logError(string.format("[MaxRoll] Ban NGON (EV=%.2f). Quay X2!", ev))
      this.hasUsedFreeReset = false
      requireSpace = 3;
      requireCoin = 2;
      protocolSubKind = 3;
      ...
      logError(string.format("[MaxRoll] Ban Trung Binh (EV=%.2f). Quay X1.", ev))
      this.hasUsedFreeReset = false
      requireSpace = 2;
      requireCoin = 1;
      protocolSubKind = 2;
